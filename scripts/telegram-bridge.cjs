...
function runAgentInSandbox(message, sessionId) {
  return new Promise((resolve) => {
    ...
    const cmd = `export NVIDIA_API_KEY=${shellQuote(API_KEY)} && export HOME=/sandbox && export OPENCLAW_HOME=/sandbox && openclaw agent --agent main --local -m ${shellQuote(message)} --session-id ${shellQuote("tg-" + safeSessionId)}`;

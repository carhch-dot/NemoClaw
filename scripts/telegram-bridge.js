...
function runAgentInSandbox(message, sessionId) {
  return new Promise((resolve) => {
    ...
    const cmd = `export NVIDIA_API_KEY=${shellQuote(API_KEY)} && export HOME=/sandbox && openclaw agent --agent main --local --raw -m ${shellQuote(message)} --session-id ${shellQuote("tg-" + safeSessionId)}`;
